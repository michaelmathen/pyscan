...
namespace pyscan {
    ...
    inline static std::tuple<Disk, double> MaxDiskRestricted(
            const pt2_t &p1, const pt2_t &p2,
            const point_list_t &net,
    ...
    inline static std::tuple<Disk, double> MaxDiskInternal(
        ...
        for (auto p1 = point_net.begin(); p1 != point_net.end() - 1; ++p1) {
            for (auto p2 = p1 + 1; p2 != point_net.end(); ++p2) {
                ...
                std::tie(local_max_disk, local_max_stat) =
                        MaxDiskRestricted(*p1, *p2, point_net, red, blue,
    ...
    std::tuple<Disk, double> max_disk(
        ...
        return MaxDiskInternal(point_net, red, blue, f);
    ...
    template <typename Pt>
    inline static std::tuple<Disk, double> MaxDiskRestrictedSimple(
            const point_list_t& point_net,
            const std::vector<Pt>& red,
            const std::vector<Pt>& blue,
            double min_dist, double max_dist,
    ...
    template<typename T>
    inline static std::tuple<Disk, double> MaxDiskScale(
            const point_list_t &point_net,
            ...
            if (net_chunk.size() >= 3) {
                for (auto pt1 = range.first; pt1 != range.second; ++pt1) {
                    for (auto &pt2: net_chunk) {
                        ...
                        double local_max_stat;
                        std::tie(local_max_disk, local_max_stat) =
                                MaxDiskRestricted(pt1->second, pt2, net_chunk, red_chunk, blue_chunk,
    ...
    template<typename Pt>
    inline static std::tuple<Disk, double> MaxDiskCachedInternal(
            const point_list_t &point_net,
            const std::vector<Pt> &red,
            const std::vector<Pt> &blue,
            const discrepancy_func_t &f) {
        ...
        for (uint32_t resolution = 2; resolution < 31; resolution++) {
            ...
            Disk local_max_disk;
            double local_max_stat;
            std::tie(local_max_disk, local_max_stat) = MaxDiskScale(point_net, red, blue, grid_r, f);
    ...
    std::tuple<Disk, double> max_disk_cached(
        ...
        return MaxDiskCachedInternal(point_net, red, blue, f);
