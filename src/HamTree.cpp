...
namespace pyscan {
    ...
    split_node_t split_directional(wpoint_list_t pts, double x, double y) {
        ...
        lower.insert(upper.end(), pts.begin(), pts.begin() + pts.size() / 2);
        upper.insert(lower.end(), pts.begin() + pts.size() / 2, pts.end());
    ...
    wpoint_list_t ham_tree_sample(const wpoint_list_t &pts, size_t s_size) {
        ...
        auto wfunc = [](const wpt2_t& pt) {
            return pt.get_weight();
        };
        while (split_queue.size() < s_size / 2) {
            pop_heap(split_queue.begin(), split_queue.end(), cmp);
            auto [upper, lower] = split_queue[split_queue.size() - 1];
            split_queue.pop_back();
            //Take a net sample.

            auto net_sample1 = random_sample_wor(upper, gen, wfunc, HAM_NET_SIZE / 2);
            auto net_sample2 = random_sample_wor(lower, gen, wfunc, HAM_NET_SIZE / 2);
            ...
            auto [plane, hTotal] = max_halfplane(net_pts, upper, lower, [](double m, double m_total, double b, double b_total ) {
                double m_val = m_total <= 0 ? .5 : m / m_total;
                double b_val = b_total <= 0 ? .5 : b / b_total;
                return std::abs(.5 - m_val) + std::abs(.5 - b_val);
            });
            (void)hTotal;
